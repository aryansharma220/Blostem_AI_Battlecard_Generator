...
import { generateSalesLayer } from "./generateSalesLayer";
...
function buildRecentLaunches(signals = {}, recentMoves = [], sources = [], sourceMap = {}) {
...
function buildBlostemWedge(battlecardDraft = {}, salesLayer = {}, pricingPosture = {}) {
  ...
  return {
    ...
    talk_track: battlecardDraft.live_call_scripts?.[0]?.script || "Use the evidence, then move to risk.",
    follow_up: battlecardDraft.live_call_scripts?.[0]?.follow_up || battlecardDraft.how_to_win?.kill_question,
...
function applyValidationToSalesLayer(salesLayer, validation) {
  ...
  return {
    ...salesLayer,
    objection_handles: (salesLayer.objection_handles || []).map((item) => ({
      ...item,
      response: needsDowngrade ? "I would verify the evidence before using that as a claim." : soften(item.response),
      downgraded: needsDowngrade,
      usage: needsDowngrade ? "ask_as_question" : "say_as_claim",
    })),
    live_call_scripts: (salesLayer.live_call_scripts || []).map((item) => ({
      ...item,
      script: needsDowngrade ? "Use this as a question, not a hard claim." : soften(item.script),
      follow_up: soften(item.follow_up),
      downgraded: needsDowngrade,
      usage: needsDowngrade ? "ask_as_question" : "say_as_claim",
    })),
    how_to_win: {
...
function buildMarkdown({ competitor, battlecard, pipeline }) {
  ...
  markdown += `## Attack Angles\n`;
  battlecard.attack_angles.forEach((angle, index) => {
    markdown += `**${index + 1}. ${angle.angle}**\n`;
    markdown += `When to use: ${angle.when_to_use}\n`;
    markdown += `What to say: "${angle.what_to_say}"\n`;
    markdown += `Close with: "${angle.close || angle.closing_question}"\n`;
    markdown += `Source: ${cite(angle.evidence_ids) || "No verified source linked"}\n\n`;
  });

  markdown += `## Objection Handling\n`;
  battlecard.objection_handles.forEach((item) => {
    markdown += `**Objection:** ${item.objection}\n`;
    markdown += `**Response:** ${item.response}\n\n`;
  });
...
export async function generateBattlecard(competitor, options = {}) {
  ...
  const salesLayerModelOutput = await generateSalesLayer({ competitor, competitorModel, core, sources, validation: { status: "ok", warnings: [] }, dealStage, skipLLM });
  ...
  const recentLaunches = buildRecentLaunches(signals, rawSalesLayer.recent_moves, sources, sourceMap);
  ...
  const battlecard = {
    ...
    objection_handles: salesLayer.objection_handles,
    live_call_scripts: salesLayer.live_call_scripts,
