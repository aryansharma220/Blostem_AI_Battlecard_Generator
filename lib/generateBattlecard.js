...
import { deduplicateAndRankSignals, deduplicateAttackAngles, reduceCompetitiveSummary, distinctCompetitorFraming } from "./competitorDeduplicator";
...
function buildRivalSenseWedge(battlecardDraft = {}, salesLayer = {}, pricingPosture = {}) {
  const comparison = salesLayer.compare_vs_us || {};
  return {
    headline: "RivalSense wins by making the buyer compare evidence, not comfort.",
    wedge: pricingPosture.rivalsense_attack || "Move the deal from feature parity to proof-backed operating risk.",
...
export async function generateBattlecard(competitor, options = {}) {
  ...
  // NEW: Deduplicate and rank signals to eliminate repetition
  const rankedSignals = deduplicateAndRankSignals(core.competitive_signals, competitorModel, 3);
  const rankedAngles = deduplicateAttackAngles(core.attack_angles).slice(0, 2);
  const rankedClaims = deduplicateAndRankSignals(core.key_claims, competitorModel, 3);

  // Apply ranked versions
  core.competitive_signals = rankedSignals;
  core.attack_angles = rankedAngles;
  core.key_claims = rankedClaims;
  ...
  const reducedSummary = reduceCompetitiveSummary(core.summary, competitor);
  core.summary = reducedSummary;
  ...
  // Apply distinct framing for the competitor
  const identity = buildCompetitorIdentity(competitor, competitorModel, sources);
  const distinctFraming = distinctCompetitorFraming(competitor, competitorModel, { unique_narrative: identity?.unique_narrative });
  ...
  battlecard.rivalsense_wedge = buildRivalSenseWedge(battlecard, salesLayer, pricingPosture);
