...
export function validateBattlecard({ sources = [], signals = {}, core = {}, salesLayer = {} }) {
  ...
  (core.key_claims || []).forEach((claim) => {
    ...
    if (!claim.ae_action) {
      pushFinding("warning", `Claim "${claim.claim}" is missing an AE action.`, "CLAIM_WITHOUT_AE_ACTION");
    }
    if (!claim.buyer_question) {
      pushFinding("warning", `Claim "${claim.claim}" is missing a buyer question.`, "CLAIM_WITHOUT_BUYER_QUESTION");
    }
  });
