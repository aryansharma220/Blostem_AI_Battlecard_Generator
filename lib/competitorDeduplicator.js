...
function scoreBlostemContrast(item, competitorModel = {}) {
  // Higher score = this directly creates a Blostem wedge
  const text = `${item.claim || item.signal || item.angle || item.point || ""} ${item.evidence || item.snippet || ""}`.toLowerCase();
...
export function deduplicateAndRankSignals(items = [], competitorModel = {}, topN = 3) {
  ...
  const scored = deduped.map((item) => ({
    ...
    blostem_contrast: scoreBlostemContrast(item, competitorModel),
  }));

  // Step 3: composite rank (weighted toward operational depth and blostem contrast)
  const ranked = scored
    .map((item) => ({
      ...item,
      rank_score: item.uniqueness_score * 0.25 + item.operational_depth * 0.5 + item.blostem_contrast * 0.25,
    }))
