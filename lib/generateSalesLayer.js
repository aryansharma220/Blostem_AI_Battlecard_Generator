...
function fallbackSalesLayer({ competitor, competitorModel, core, sources, validation, dealStage, riskDash = {} }) {
  ...
  return {
    ...
    discovery_angles: discoveryAngles.length > 0 ? discoveryAngles : [
      {
        angle: identity.risk_label,
        discovery_question: identity.opening_question,
        implication: identity.unique_narrative,
        why_it_works: identity.tension,
        evidence_sources: "github, status-page, job-postings, operational-maturity-signals",
      },
    ],

    // NEW: Procurement Conversation Sequence (risk-based discovery)
    procurement_conversation_sequence: [
      {
        moment: "Opening (Discovery)",
        what_to_verify: identity.opening_question,
        why: identity.unique_narrative,
        follow_up: `Which matters more here: ${identity.pricing_label.toLowerCase()} or ${identity.proof_label.toLowerCase()}?`,
      },
      {
        moment: "Mid-Call (Risk surfaces)",
        what_to_verify: `Which part of the ${identity.archetype} story needs proof in your environment?`,
        why: identity.tension,
        follow_up: `When does ${identity.risk_label.toLowerCase()} become visible in your workflow?`,
      },
      {
        moment: "Closing (If engaged)",
        what_to_verify: `What would make ${competitor} harder to keep than to replace?`,
        why: identity.summary,
        follow_up: "How long would full migration take? What would speed it up?",
      },
    ],
    ...
    deal_context: {
      ...
      default_opening: identity.opening_question,
    },
