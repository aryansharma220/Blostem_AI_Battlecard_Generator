...
import { buildRivalSenseComparison } from "./blostemProfile";
...
function fallbackSalesLayer({ competitor, competitorModel, core, sources, validation, dealStage, riskDash = {} }) {
  ...
  return {
    ...
    compare_vs_us: buildRivalSenseComparison(competitorModel),
    market_reality: {
...
async function generateStrategy({ competitor, competitorModel, core, dealStage }) {
  ...
  const prompt = `You are a senior enterprise GTM strategist preparing a RivalSense AE to compete against ${competitor}.
...
export async function generateSalesLayer(input) {
  ...
  return {
    ...
    compare_vs_us: buildRivalSenseComparison(input.competitorModel || {}),
