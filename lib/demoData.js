...
const DIRECTIONAL_DEMO_SOURCES = {
  razorpay: [
    {
      title: "Razorpay Careers: Risk Operations and Reconciliation Roles",
      url: "https://razorpay.com/jobs/",
      published_at: "2025-02-20",
      snippet:
        "Open roles around risk operations, settlements, and reconciliation suggest scale-control workstreams worth validating in procurement.",
      type: "social",
      raw_metadata: { job_posting: true, hiring_intent_signal: true },
    },
    {
      title: "Razorpay Status Page: Incident History",
      url: "https://status.razorpay.com/",
      published_at: "2025-02-18",
      snippet:
        "Public status history creates operational evidence for uptime, incident handling, and resolution-process diligence.",
      type: "company",
      raw_metadata: { status_page: true, operational_signal: true },
    },
    {
      title: "Merchant Forum Thread: Settlement Reconciliation Friction",
      url: "https://demo.blostem.local/community/razorpay-settlement-reconciliation",
      published_at: "2025-02-02",
      snippet:
        "Directional merchant discussion mentions reconciliation effort, settlement timing questions, and support escalation uncertainty.",
      type: "forum",
  ...
  paytm: [
    {
      title: "Paytm Careers: Merchant Operations Hiring",
      url: "https://paytm.com/careers/",
      published_at: "2025-02-16",
      snippet:
        "Hiring emphasis around merchant operations and support suggests operational capacity is a diligence area, not a feature point.",
      type: "social",
      raw_metadata: { job_posting: true, hiring_intent_signal: true },
    },
    {
      title: "Regulatory News Archive: Paytm Payments Bank Restrictions",
      url: "https://rbi.org.in/",
      published_at: "2024-01-31",
      snippet:
        "Regulatory history around the broader Paytm ecosystem should be separated from current merchant gateway claims during procurement review.",
      type: "news",
      raw_metadata: { regulatory_signal: true, compliance_history: true },
    },
    {
      title: "Merchant Review Cluster: Support and Settlement Questions",
      url: "https://demo.blostem.local/reviews/paytm-merchant-support",
      published_at: "2025-02-07",
      snippet:
        "Directional review cluster references support response, settlement clarity, and merchant escalation paths.",
      type: "reviews",
  ...
  phonepe: [
    {
      title: "PhonePe Careers: Platform Reliability and Data Roles",
      url: "https://www.phonepe.com/careers/",
      published_at: "2025-02-12",
      snippet:
        "Hiring signal around platform reliability and data operations suggests scale, observability, and control are relevant buyer diligence themes.",
      type: "social",
      raw_metadata: { job_posting: true, hiring_intent_signal: true },
    },
    {
      title: "PhonePe Status and Trust Signals",
      url: "https://www.phonepe.com/trust-and-safety/",
      published_at: "2025-02-01",
      snippet:
        "Trust messaging should be tested against account-specific uptime, support, settlement, and incident evidence.",
      type: "company",
      raw_metadata: { status_page: true, operational_signal: true },
    },
    {
      title: "Merchant Forum Thread: UPI Reliability and Support Escalation",
      url: "https://demo.blostem.local/community/phonepe-upi-reliability",
      published_at: "2025-02-04",
      snippet:
        "Directional discussion mentions UPI reliability, support escalation, and issue resolution during high-volume periods.",
      type: "forum",
      raw_metadata: { community_discussion: true, unfiltered_sentiment: true },
    },
  ],
...
// Ensure demo sources include the competitor name when no curated demo exists
export function getDemoSources(competitor, query = "") {
  const key = keyFor(competitor);
  const selected = [
    ...(DEMO_DATA[key] || DEFAULT_DEMO_SET),
    ...(DIRECTIONAL_DEMO_SOURCES[key] || []),
  ];

  // When using the generic default set, inject the competitor name into titles/snippets
  const injected = selected.map((source) => {
    if (!DEMO_DATA[key]) {
      ...
      return {
        ...
        snippet: `${source.snippet} (Demo data seeded for ${name})`,
      };
    }
    return source;
  });

  // Mark all demo sources as synthetic so UI can label them clearly
  const markedSources = injected.map((source) => ({
    ...source,
    is_synthetic: true,
  }));

  // Add query-aware synthetic note so generation still feels contextual.
  if (!query) return markedSources;
  return [
    ...markedSources,
    {
      title: `Demo Query Context for ${String(competitor)}`,
